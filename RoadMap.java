...
public class RoadMap {
	...
	public static class DijkstraShortestPath {
		public void computeShortestPaths(Vertex sourceVertex) {
			sourceVertex.setDistance(0); // Setting Source vertex distance to 0
			PriorityQueue<Vertex> priorityQueue = new PriorityQueue<Vertex>();
			priorityQueue.add(sourceVertex); // Adding source vertex to the PQ.
			sourceVertex.setVisited(true); // Marking the source vertex as visited.

			while (!priorityQueue.isEmpty()) {
				Vertex actualVertex = priorityQueue.poll(); // Removing the source vertex from the PQ and storing it in actualVertex

				for (Edge edge : actualVertex.getIncidentRoads()) {
					Vertex v = edge.getSecondVertex(); // Getting the adjacent vertex's connected to the source.
					if (!v.isVisited()) {
						int newDistance = actualVertex.getDistance() + edge.getLength(); // Updating the distance
	...
	public boolean BFS(Vertex startVertex, Vertex endVertex) {
		...
		while(!adjacent.isEmpty()) {
			...
			for (Edge neighbour : current.getIncidentRoads()) {
				if(neighbour.getSecondVertex() == endVertex) {
					return true;
				}
				Vertex neighbourVertex = neighbour.getSecondVertex();
				if(neighbourVertex.hasChargingStation() && !visited.contains(neighbourVertex)) {
					// Adding vertices with charging stations and have not been visited to the queue
					adjacent.add(neighbourVertex);
